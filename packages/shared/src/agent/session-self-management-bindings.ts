 ...
 *
 * Attaches session callback-backed properties to a SessionToolContext using
 ...
 *
 * Defines lazy getters for session self-management, inter-session messaging,
 * shared terminal access, source activation, and messaging gateway bindings.
 *
...
export function attachSessionSelfManagementBindings(
  ...
  Object.defineProperty(context, 'sharedTerminal', {
    get() {
      return getSessionScopedToolCallbacks(sessionId)?.sharedTerminalFns;
    },
    configurable: true,
    enumerable: true,
  });
